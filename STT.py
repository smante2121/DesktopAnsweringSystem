...
is_finals = [] # list to store final results
wait_for_activation = asyncio.Event() # event to signal STT to start capturing


async def speech_to_text():  # method to convert speech to text # added capture complete
    global is_finals
    is_finals = []
    last_utterance_end_time = None # variable to store the time of the last utterance end
    utterance_end_threshold = 2 # threshold for utterance end
    ...
    try:
        ...
        async def on_utterance_end(self, utterance_end, **kwargs):  # method to handle utterance end
            nonlocal last_utterance_end_time
            logging.info(f"Utterance End")
            last_utterance_end_time = time.time() # update the last utterance end time


        async def check_for_silence(): # method to check for silence
            nonlocal last_utterance_end_time
            while True: # check for silence
                if last_utterance_end_time is not None and time.time() - last_utterance_end_time > utterance_end_threshold:
                    await shutdown(SIGINT, loop, dg_connection, stream)  # Trigger shutdown on prolonged silence
                await asyncio.sleep(1) # check every second

        async def on_close(self, close, **kwargs):  # method to handle connection close
            logging.info(f"Connection Closed")
            utterance = " ".join(is_finals)
            with open(TEXT_FILE, "a") as final_text_file: # write the final text to the file
        ...
        asyncio.create_task(check_for_silence()) # start the check for silence task
